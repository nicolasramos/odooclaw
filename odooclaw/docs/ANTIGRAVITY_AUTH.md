...
10. [Creating a New Provider in OdooClaw](#creating-a-new-provider-in-odooclaw)
...
type ProviderAuthResult = {
  ...
  configPatch?: Partial<OdooClawConfig>;
  defaultModel?: string;
  notes?: string[];
};
```
...
- Go ≥ 1.21
- OdooClaw codebase (`pkg/providers/` and `pkg/auth/`)
...
Auth profiles are stored in `~/.odooclaw/auth.json`:
...
odooclaw auth login --provider your-provider

# List models (for Antigravity)
odooclaw auth models

# Start the gateway
odooclaw gateway

# Run an agent with a specific model
odooclaw agent -m "Hello" --model your-model
```
...
export ODOOCLAW_AGENTS_DEFAULTS_MODEL=your-model

# Override provider settings
export ODOOCLAW_MODEL_LIST='[{"model_name":"your-model","model":"your-provider/model-name","api_key":"..."}]'
```
...
- **Source Files:**
  ...
  - `cmd/odooclaw/cmd_auth.go` - Auth CLI commands
