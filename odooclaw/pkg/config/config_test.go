...
func TestDefaultConfig_WorkspacePath_Default(t *testing.T) {
	...
	t.Setenv("ODOOCLAW_HOME", "")
	// Set a known home for consistent test results
	t.Setenv("HOME", "/tmp/home")

	cfg := DefaultConfig()
	want := filepath.Join("/tmp/home", ".odooclaw", "workspace")
...
func TestDefaultConfig_WorkspacePath_WithOdooclawHome(t *testing.T) {
	t.Setenv("ODOOCLAW_HOME", "/custom/odooclaw/home")

	cfg := DefaultConfig()
	want := "/custom/odooclaw/home/workspace"

	if cfg.Agents.Defaults.Workspace != want {
		t.Errorf("Workspace path with ODOOCLAW_HOME = %q, want %q", cfg.Agents.Defaults.Workspace, want)
	}
}
