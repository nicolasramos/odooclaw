...
class MailThread(models.AbstractModel):
    ...
    def message_post(self, **kwargs):
        ...
        odooclaw_user = self.env.ref('mail_bot_odooclaw.odooclaw_bot', raise_if_not_found=False)
        ...
        if message.model == 'discuss.channel':
            ...
            if channel.channel_type == 'chat' and odooclaw_partner_id in channel.channel_member_ids.mapped('partner_id').ids:
                is_dm = True
        ...
        if is_mentioned or is_dm:
            ...
            if message.attachment_ids:
                body_text += "\n\n[Adjuntos en este mensaje:]\n"
                for att in message.attachment_ids:
                    body_text += f"- {att.name} (ID: {att.id})\n"
            ...
            webhook_url = self.env['ir.config_parameter'].sudo().get_param('odooclaw.webhook_url', 'http://odooclaw:18790/webhook/odoo')
            ...
            threaded_call = threading.Thread(target=send_webhook, args=(webhook_url, payload))
            threaded_call.start()
